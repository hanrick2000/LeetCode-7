...
public:
    ...
    //  }  
     ...
     //合并，修改num的值  
     void Union(vector<int> &father, vector<int> &size ,int a,int b)  {   
    ...
    int numIslands(vector<vector<char>>& grid) {
            ...
            for(int j = 0; j < grid[0].size(); ++j) {
                if(grid[i][j]=='1') {
                    ...
                    if(i + 1 < grid.size() && grid[i+1][j] == '1')  {
                        Union(father, size, index, index+grid[0].size());
                    }
                    if(j + 1 < grid[0].size() && grid[i][j+1] == '1') {
                        Union(father, size, index, index+1);
                    }
                }
            }
        }
        ...
        for(int i = 0; i < grid.size(); ++i) {
            for(int j = 0; j < grid[0].size(); ++j) {
                int index = i*grid[0].size() + j;
                if(grid[i][j] == '1' && findSet(index, father) == index) ++count;
            }
        }
